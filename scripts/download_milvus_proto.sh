...
API_VERSION=$(go list -m github.com/shaoting-huang/milvus-proto/go-api/v2@v2.0.0-20241024101215-1b5e6603c257 | awk -F' ' '{print $2}')

if [ ! -d "$THIRD_PARTY_DIR/milvus-proto" ]; then
  mkdir -p $THIRD_PARTY_DIR
  pushd $THIRD_PARTY_DIR
  git clone https://github.com/shaoting-huang/milvus-proto.git
  cd milvus-proto
  # try tagged version first
  COMMIT_ID=$(git ls-remote https://github.com/shaoting-huang/milvus-proto.git refs/tags/${API_VERSION} | cut -f 1)
  if [[ -z $COMMIT_ID ]]; then
