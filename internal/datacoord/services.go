...
func (s *Server) CreateSnapshot(ctx context.Context, req *datapb.CreateSnapshotRequest) (*commonpb.Status, error) {
	...
	// Check if snapshot name already exists (global uniqueness)
	_, err := s.meta.GetSnapshotMeta().GetSnapshot(ctx, req.GetName())
	if err == nil {
		// Snapshot name already exists
		err := merr.WrapErrParameterInvalidMsg(fmt.Sprintf("snapshot name %s already exists", req.GetName()))
		log.Warn("snapshot name already exists", zap.String("name", req.GetName()))
		return merr.Status(err), nil
	}
