...
// StorageV2SegmentManifest records the mapping between segment ID and its StorageV2 manifest path.
...
func (w *SnapshotWriter) Save(ctx context.Context, snapshot *SnapshotData) (string, error) {
	...
	// Step 1.5: Collect StorageV2 manifest paths from segments.
	...
	metadataFilePath, _, err := w.writeMetadataFile(ctx, basePath, writeUUID, snapshot, manifestPaths, storagev2Manifests)
	if err != nil {
...
func (w *SnapshotWriter) writeMetadataFile(ctx context.Context, basePath string, writeUUID string, snapshot *SnapshotData, manifestPaths []string, storagev2Manifests []*StorageV2SegmentManifest) (string, string, error) {
...
func (r *SnapshotReader) ReadSnapshot(ctx context.Context, collectionID int64, snapshotID int64, includeSegments bool) (*SnapshotData, error) {
	...
	if includeSegments {
		...
		// Step 3.5: Populate manifest_path from StorageV2ManifestList
