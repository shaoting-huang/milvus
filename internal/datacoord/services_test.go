...
func TestServer_CreateSnapshot_DuplicateName(t *testing.T) {
	t.Run("snapshot name already exists", func(t *testing.T) {
		...
		// Create existing snapshot info
		existingSnapshot := &datapb.SnapshotInfo{
			Id:           1,
		...
		// Setup snapshot meta with existing snapshot
		snapshotMeta := &snapshotMeta{
			snapshotID2DataInfo: typeutil.NewConcurrentMap[UniqueID, *SnapshotDataInfo](),
		}
		snapshotMeta.snapshotID2DataInfo.Insert(1, &SnapshotDataInfo{
			snapshotInfo: existingSnapshot,
