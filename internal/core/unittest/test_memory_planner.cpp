...
milvus_storage::RowGroupMetadataVector CreateRowGroupMetadataVector(const std::vector<int64_t>& memory_sizes_mb) {
    std::vector<milvus_storage::RowGroupMetadata> metadatas;
    for (auto size : memory_sizes_mb) {
        metadatas.emplace_back(size * 1024 * 1024, 0, 0); // Convert MB to bytes
    }
...
TEST(SplitRowGroupsTest, EmptyInput) {
    ...
    auto result = split_row_groups(empty_input, metadatas);
...
TEST(SplitRowGroupsTest, SingleRowGroup) {
    ...
    auto result = split_row_groups(input, metadatas);
    EXPECT_EQ(result.size(), 1);
    EXPECT_EQ(result[0], (RowGroupBlock{0, 1}));
}

TEST(SplitRowGroupsTest, ContinuousWithinMemoryLimit) {
...
TEST(SplitRowGroupsTest, ContinuousExceedMemoryLimit) {
