...
SealedLoadFieldData(const GeneratedData& dataset,
    ...
    for (auto& field_data : dataset.raw_->fields_data()) {
        ...
        auto field_data_ptr = CreateFieldDataFromDataArray(row_count, &field_data, field_meta);
        auto arrow_data_wrapper = storage::ConvertFieldDataToArrowDataWrapper(field_data_ptr);
...
ConvertToArrowRecordBatch(const GeneratedData& dataset, int64_t dim, const std::shared_ptr<arrow::Schema>& arrow_schema) {
    ...
    for (int i = 0; i < arrow_schema->num_fields(); ++i) {
        ...
        auto field_id = std::stol(field->metadata()->Get(milvus_storage::ARROW_FIELD_ID_KEY).ValueOrDie());
        
        auto it = field_data_map.find(field_id);
        if (it != field_data_map.end()) {
            auto field_meta = dataset.schema_->operator[](FieldId(field_id));
            auto field_data_ptr = CreateFieldDataFromDataArray(dataset.raw_->num_rows(), it->second, field_meta);
            auto arrow_data_wrapper = storage::ConvertFieldDataToArrowDataWrapper(field_data_ptr);
    ...
    return arrow::RecordBatch::Make(arrow_schema, dataset.raw_->num_rows(), arrays);
}
